    ...
    <>
      <header className="h-14 border-b border-hairline bg-surface-1/95 backdrop-blur sticky top-0 z-30 select-none">
        <div className="w-full max-w-7xl mx-auto h-full px-4 sm:px-6 lg:px-8 flex items-center justify-between gap-4">
          {/* Left: Brand Logo + Horizontal Nav Items aligned together */}
          <div className="flex items-center gap-6 lg:gap-8 min-w-0">
            {/* Mobile menu trigger */}
            <button
              onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
              className="lg:hidden p-1.5 rounded border border-hairline bg-surface-2 text-ink-muted hover:text-ink shrink-0"
              aria-label="Menu"
            >
            ...
            {/* Horizontal Navigation Tabs (Never wrapped, comfortable breathing room) */}
              ...
              {NAV_ITEMS.map((item) => {
                ...
                return (
                  <Link
                    ...
                    href={item.href}
                    className={clsx(
                      "flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap shrink-0 transition-all duration-150",
                      isActive
          ...
          {/* Right Utilities: Search + Theme Toggle + User Info */}
          <div className="flex items-center gap-2.5 shrink-0">
            {/* Quick Search Button (Ctrl+K) */}
            <button
              onClick={() => setIsCommandOpen(true)}
              className="flex items-center gap-2 bg-surface-2 hover:bg-surface-3 border border-hairline hover:border-hairline-strong rounded-lg px-2.5 py-1.5 text-xs text-ink-muted transition-colors shadow-2xs group shrink-0"
            ...
            >
              ...
              <span className="hidden sm:inline text-ink-subtle group-hover:text-ink-muted whitespace-nowrap text-xs">
                Rechercher...
              </span>
            ...
            {/* Live Sync Indicator */}
            <div className="hidden xl:flex items-center gap-1.5 px-2 py-1 rounded bg-accent-emerald/10 border border-accent-emerald/20 text-accent-emerald text-[11px] font-mono shrink-0">
            ...
            <div className="hidden sm:flex items-center gap-2 pl-2 border-l border-hairline shrink-0">
              <div className="w-7 h-7 rounded-lg bg-surface-3 border border-hairline-strong flex items-center justify-center text-ink font-mono text-xs font-semibold shrink-0">
                K
              </div>
        ...
        {mobileMenuOpen && (
          <div className="lg:hidden border-b border-hairline bg-surface-1 px-4 py-3 space-y-1 shadow-xl animate-in slide-in-from-top-2">
            {NAV_ITEMS.map((item) => {
              const Icon = item.icon;
              const isActive = pathname === item.href;
